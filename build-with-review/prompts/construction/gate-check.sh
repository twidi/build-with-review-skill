...
validate_result() {
    ...
    local proved_code=${8:--} historical=${9:-false} result code_proof=- tree
    validate_gate
    ...
    if [ "$mode" = task ] || [ "$mode" = correction-task ]; then
        ...
        if [ "$historical" = true ] && [ "$mode" = task ]; then
...
if correction_mode:
    identity |= {"correction"} | correction_authority
if "execution" in d:
...
PY
    ...
    validate_result "$op" "$scope" "$lot" "$task" "$attempt" "$head" "$correction" - true
    printf '%s\n' "$op"
}
...
PY
    ...
    validate_result "$op" "$scope" "$lot" "$task" "$attempt" "$commit" "$correction" - true
    printf '%s\n' "$fields"
}
...
case ${1:-} in
    ...
    require-task-history)
        ...
        if [ "$8" = - ]; then
            validate_result "$2" task "$3" "$4" "$5" "$6" - "$7" true
        else
            validate_result "$2" correction-task "$3" "$4" "$5" "$6" "$8" "$7" true
        fi
        printf 'ACCEPTED %s\n' "$2"
        ;;
    require-review)
